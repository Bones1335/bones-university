...
CREATE_DEGREE_JSNOW=$(curl -X POST http://localhost:8080/admin/degrees -H "Authorization: Bearer $jsToken" -d '{"degree_name":"Physical Therapy","degree_level":"Masters","degree_department":"Rehabilitation Department","degree_duration":4}')
...
CREATE_YEAR_JSNOW=$(curl -X POST http://localhost:8080/admin/years -H "Authorization: Bearer $jsToken" -d '{"year_in_school":1}')

echo $CREATE_YEAR_JSNOW | jq .
